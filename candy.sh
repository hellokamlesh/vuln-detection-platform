...
run_httpx() {
	if [[ "$USE_HTTPX" == "true" ]]; then
		...
		local screenshot_timeout="${CANDY_SCREENSHOT_TIMEOUT:-20}"
		if ! [[ "$screenshot_timeout" =~ ^[0-9]+$ ]] || (( screenshot_timeout <= 0 )); then
			screenshot_timeout=20
		fi

		local chrome_preference="${CANDY_HTTPX_SYSTEM_CHROME:-auto}"
		local chrome_flag=""
		if [[ "$chrome_preference" != "off" ]]; then
			...
			if [[ -z "$chrome_flag" && "$chrome_preference" == "require" ]]; then
				warning "CANDY_HTTPX_SYSTEM_CHROME is set to require but no local Chrome/Chromium binary was found. Falling back to bundled renderer."
			fi
		fi
