...
CANDY_SCRIPT = REPO_ROOT / "candy.sh"
...
class JobManager:
    def __init__(self, max_workers: int, record_callback):
        ...
        self.running: Dict[str, CandyJob] = {}
        self.queue: List[CandyJob] = []
        self.recent: Deque[CandyJob] = deque(maxlen=QUEUE_HISTORY_LIMIT)
        self.event = threading.Event()
        self.scheduler_thread = threading.Thread(target=self._scheduler_loop, daemon=True)
        self.scheduler_thread.start()

    def submit(self, job: CandyJob, start_mode: str) -> Tuple[CandyJob, str]:
        now = utc_now()
        with self.lock:
    ...
    def _queue_message(self, job: CandyJob) -> str:
    ...
    def _scheduler_loop(self) -> None:
        while True:
            job_to_start: Optional[CandyJob] = None
    ...
    def _pop_ready_job_locked(self) -> Optional[CandyJob]:
        now = utc_now()
    ...
    def _start_job_locked(self, job: CandyJob) -> None:
        job.status = "running"
        job.status_message = "candy.sh started"
    ...
    def _run_job(self, job: CandyJob) -> None:
        ...
        try:
            ...
            env = os.environ.copy()
            cmd = ["bash", str(CANDY_SCRIPT), str(job.targets_file)]
            ...
            if job.cancel_event.is_set():
                ...
                job.error_message = f"candy.sh exited with {return_code}."
...
def create_app() -> Flask:
    ...
    def record_run(job: CandyJob, status: str, error: Optional[str], log_path: Optional[Path]) -> None:
        project_dir = PROJECTS_DIR / job.project_slug
        project_dir.mkdir(parents=True, exist_ok=True)
        metadata = load_metadata(project_dir)
    ...
    max_concurrent = max(1, int(os.getenv("CANDY_MAX_CONCURRENT", "1")))
