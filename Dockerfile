...
RUN python3 -m venv /opt/candy/.venv \
 && /opt/candy/.venv/bin/pip install --upgrade pip \
 && /opt/candy/.venv/bin/pip install --no-cache-dir -r requirements.txt

COPY . .

RUN sed -i 's/\r$//' candy.sh || true \
 && chmod 0755 candy.sh entrypoint.sh

COPY --from=builder /out/* /usr/local/bin/
ENV PATH=/opt/candy/.venv/bin:/usr/local/bin:$PATH
ENV XDG_CACHE_HOME=/opt/candy/.cache

RUN mkdir -p /opt/candy/output /opt/candy/.cache

ENV CANDY_WEB_PORT=8787
EXPOSE 8787
